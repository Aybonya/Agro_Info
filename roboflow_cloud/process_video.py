"""
Оптимизированная фиксация проездов транспорта через Roboflow Cloud API.
Формат "1 проезд = 1 карточка ТС":
- vehicle_card.jpg: единое фото с врезкой номера (Picture-in-Picture) и плашкой.
- license_plate.jpg: кроп номера для систем распознавания.
- info.json: метаданные проезда.
Фильтрует случайные мигания и одиночные кадры без номера.
"""
...
def process_video_roboflow(
    ...
    print(f"\n[ROBOFLOW] Видео: {video_w}x{video_h}, Длина: {format_timestamp(duration_sec)}")
    ...
    frame_idx = 0
    vehicle_counter = 0
    current_session: CloudPassageSession | None = None
    saved_passages = []

    temp_frame_path = output_dir / "_temp_infer_frame.jpg"
    ...
    while cap.isOpened():
        ...
        try:
            res = client.infer(str(temp_frame_path), model_id=MODEL_ID)
        ...
        for pred in res.get("predictions", []):
            ...
            if score < conf_thresh:
                continue
            cls_name = pred.get("class", "object").lower()
            xc, yc = float(pred["x"]), float(pred["y"])
            w, h = float(pred["width"]), float(pred["height"])

            x1 = max(0, int(xc - w / 2))
            y1 = max(0, int(yc - h / 2))
            x2 = min(video_w, int(xc + w / 2))
            y2 = min(video_h, int(yc + h / 2))
        ...
        if has_interest:
            if current_session is None:
                vehicle_counter += 1
                current_session = CloudPassageSession(vehicle_counter, current_time_sec, frame_idx)
    ...
    print("\n" + "=" * 65)
    print(f"[ИТОГ] Завершено! Подтверждено реальных ТС: {len(saved_passages)} шт.")
...
def main():
    parser = argparse.ArgumentParser(description="Оптимизированная детекция ТС через Roboflow Cloud")
    parser.add_argument("source", nargs="?", default="https://www.youtube.com/watch?v=NL0CO28eq9Q")
    ...
    parser.add_argument("--all-vehicles", action="store_true", default=False)
    args = parser.parse_args()

    process_video_roboflow(
        source=args.source,
